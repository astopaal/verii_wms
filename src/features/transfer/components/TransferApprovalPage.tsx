        ...
        <CardContent>
          <div className="hidden md:block rounded-md border">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{t('transfer.approval.id', 'ID')}</TableHead>
                  <TableHead>{t('transfer.approval.documentNo', 'Belge No')}</TableHead>
                  <TableHead>{t('transfer.approval.documentDate', 'Belge Tarihi')}</TableHead>
                  <TableHead>{t('transfer.approval.customerCode', 'Cari Kodu')}</TableHead>
                  <TableHead>{t('transfer.approval.customerName', 'Cari Adı')}</TableHead>
                  <TableHead>{t('transfer.approval.sourceWarehouse', 'Çıkış Deposu')}</TableHead>
                  <TableHead>{t('transfer.approval.targetWarehouse', 'Varış Deposu')}</TableHead>
                  <TableHead>{t('transfer.approval.completionDate', 'Tamamlanma Tarihi')}</TableHead>
                  <TableHead>{t('transfer.approval.actions', 'İşlemler')}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {data?.data && data.data.length > 0 ? (
                  data.data.map((item: AwaitingApprovalHeader) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>{item.sourceWarehouseName || item.sourceWarehouse || '-'}</TableCell>
                      <TableCell>{item.targetWarehouseName || item.targetWarehouse || '-'}</TableCell>
