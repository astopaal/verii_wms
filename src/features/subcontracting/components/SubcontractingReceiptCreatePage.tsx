...
export function SubcontractingReceiptCreatePage(): ReactElement {
  ...
  useEffect(() => {
    setPageTitle(t('subcontracting.receipt.create.title', 'Yeni Fason Giriş Emri'));
    return () => {
      setPageTitle(null);
    };
  }, [t, setPageTitle]);
  ...
  const createMutation = useMutation({
    mutationFn: async (formData: SubcontractingFormData) => {
      return subcontractingApi.createSubcontractingReceipt(formData, selectedItems);
    },
    onSuccess: () => {
    ...
    onError: (error: Error) => {
      toast.error(
        error.message || t('subcontracting.receipt.create.error', 'Fason giriş emri oluşturulurken bir hata oluştu')
      );
    },
  });
