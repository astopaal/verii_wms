        ...
        <CardContent className="flex flex-col items-center justify-center py-12">
          <div className="text-center space-y-4">
            ...
            <div className="space-y-2">
              <CardTitle>{t('shipment.step2.noOrderSelected', 'Sipariş Seçimi')}</CardTitle>
              <CardDescription>
                {t('shipment.step2.selectOrderPrompt', 'İşlem yapmak için siparişler sekmesinden bir sipariş seçiniz.')}
              </CardDescription>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-sm text-muted-foreground">
            {t('shipment.step2.noItemsFound', 'Bu siparişte ürün bulunamadı.')}
          </p>
        </CardContent>
      </Card>
    );
    ...
    <div className="flex flex-col h-full">
      <div className="pb-2 space-y-2 border-b px-2 shrink-0">
        <div className="flex items-center justify-between gap-2">
          <div>
            <h3 className="font-semibold text-sm">{t('shipment.step2.orderContent', 'Sipariş İçeriği')}</h3>
            <p className="text-xs text-muted-foreground">
              {t('shipment.step2.itemsCount', '{{count}} kalem ürün', { count: totalItems })}
            </p>
          </div>
          <div className="text-right">
            <p className="text-xs text-muted-foreground">{t('shipment.step2.started', 'Başlanan')}</p>
